...
const CreateEvent = () => {
  ...
  // Carregar sugestões de restaurantes favoritos
  useEffect(() => {
    const loadFavoriteSuggestions = async () => {
      if (!user?.id) return;
      
      try {
        const result = await getFavoritesForSuggestions();
        if (result.success && result.suggestions) {
          setFavoriteSuggestions(result.suggestions);
          console.log(`✅ ${result.suggestions.length} restaurantes favoritos carregados para sugestões`);
        }
      } catch (error) {
        console.error('❌ Erro ao carregar sugestões de favoritos:', error);
      }
    };
